...
(function($) {
  ...
  $(document).ready(function(){
        ...
        var objPrimeiro = $(".horizontal-tab-button").first();
        var objUltimo   = $(".horizontal-tab-button").last();
        var objAtual    = $(".horizontal-tab-button.selected");
        
        console.log($("#user-profile-form").find(".horizontal-tab-button").last().text());
        $("#user-profile-form").find(".horizontal-tab-button").last().removeClass("last").addClass('ultimo');
        $("#user-profile-form").find(".horizontal-tab-button-5").addClass('last');
        ...
        $(".btn-proximo").click(function(){
            ...
            if($(objAtual).hasClass("last")){
                $(".horizontal-tab-button.first").find("a").trigger("click");
            }else
                $(objAtual).next().find("a").trigger("click");
            
             $('html, body').animate({
                scrollTop: $(objAtual).offset().top
            }, 200);
        });
        
        $("#user-profile-form").submit(function(){
            
            if($("#user-profile-form").find(".horizontal-tab-button.selected").hasClass("last")){
                $("#user-profile-form").attr("action", $("#user-profile-form").attr("action") + "?destination=user");
            }
